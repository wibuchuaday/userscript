...
(async () => {
    ...
    document.documentElement.innerHTML = `<html><head><title>TienBypass USERSCRIPT</title><link rel="stylesheet" href="https:///TienBypass/assets/css/styles.css"></head><body class="userscript"><h1>TienBypass userscript</h1><h2>redirecting...</h2></body></html>`;

    const urlParams = new URLSearchParams(window.location.search);
    const redirectUrl = urlParams.get('redirect')

    if (redirectUrl && redirectUrl.includes('https://flux.li/android/external/main.php')) {
        document.body.innerHTML = `<h1>TienBypass userscript</h1><h2>Fluxus implements some extra security checks to detect bypasses so we can't automatically redirect you.</h2><h3><a href="${redirectUrl}">Click here to redirect</a></h3>`;
        return;
    } else if(redirectUrl) {
        location.href = redirectUrl
        return;
    }
    location.href = `https://TienBypass/userscript.html?url=${encodeURIComponent(location.href)}&time=${config.time}&key=${config.key}`
    return;
})();
